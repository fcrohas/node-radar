          ...
          if (plane.ICAO == msg.ICAO) {
            ...
            if ((!isUndefinedOrNull(msg.latitude)) && ((msg.latitude != plane.latitude) || (msg.longitude!=plane.longitude)))  {
              ...
              if (plane.show) {
                ...
                // Get previous value
                var bearing = Math.abs(PlaneService.getBearing(plane.latitude, plane.longitude, msg.latitude, msg.longitude) - plane.icon.rotation);
                var delta_altitude = Math.abs(plane.altitude - msg.altitude);
                // Reduce point using bearing and altitude
                if ((bearing > 5) || (delta_altitude > 250) || (plane.trackhistory.length == 0)) {
                  plane.trackhistory.push( { id : plane.trackhistory.length, 
                  ...
                  plane.trackhistory[plane.trackhistory.length -1].track[1].longitude = msg.longitude;
                  plane.trackhistory[plane.trackhistory.length -1].track[1].latitude = msg.latitude;
                }          
              }
...
// Manage quality signal
  ...
  return {
    ...
    getPlaneInfo : function(plane) {
      ...
      PlaneService.getTrackHistory(plane.ICAO).then(function(track) {
        plane.trackhistory = track.trackhistory;
      });
    ...
    getTrackHistory : function() {
      var track = [];
      var id = -1;
      angular.forEach(planes, function(plane) {
          if (plane.show) {
            id = plane.ICAO;
            track = plane.trackhistory;
          }          
      });
      return track;
    }
